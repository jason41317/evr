    ...
    function transaction($txn = null,$id_filter=null) {
        switch ($txn){
            ...
            case 'export':
                ...
                $excel->getActiveSheet()->setCellValue('A6','Item Transfer')
            ...
            case 'export-product':
                ...
                $m_company_info=$this->Company_model;
                $m_issuance_department_item=$this->Issuance_department_item_model;
                ...
                $company_info=$m_company_info->get_list();
                $data['company_info']=$company_info[0];
                $items=$m_issuance_department_item->per_product($from, $to);
                ...
                //name the worksheet
                $excel->getActiveSheet()->setTitle("Item Transfer per Product");
                $excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(TRUE);
                $excel->getActiveSheet()->mergeCells('A1:B1');
                $excel->getActiveSheet()->mergeCells('A2:C2');
                $excel->getActiveSheet()->mergeCells('A3:B3');
                ...
                $excel->getActiveSheet()->setCellValue('A6','Item Transfer per Product')
                                        ->getStyle('A6')->getFont()->setBold(TRUE);
                $excel->getActiveSheet()->setCellValue('A7','Period : '.date('F d, Y', strtotime($from)).' to '.date('F d, Y', strtotime($to)))
                                        ->getStyle('A7')->getFont()->setItalic(TRUE);
                $excel->getActiveSheet()->setCellValue('A8','')
                ...
                foreach ($items as $item) {
                    $excel->getActiveSheet()
                        ->setCellValue('A'.$i,$item->trn_no)
                        ->setCellValue('B'.$i,$item->from_department)
                        ->setCellValue('C'.$i,$item->to_department)
                        ->setCellValue('D'.$i,$item->date_issued)
                        ->setCellValue('E'.$i,$item->product_desc)
                        ->setCellValue('F'.$i,$item->product_type)
                        ->setCellValue('G'.$i,$item->issue_qty)
                        ->setCellValue('H'.$i,$item->unit_name)
                        ->setCellValue('I'.$i,$item->exp_date)
                        ->setCellValue('J'.$i,$item->batch_no)
                        ->setCellValue('K'.$i,$item->issue_price)
                        ->setCellValue('L'.$i,$item->issue_line_total_price)
                        ->setCellValue('M'.$i,$item->remarks);
                    $i++;
                }
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment;filename="Item Transfer per Product '.date("F d, Y", strtotime($from)).' to '.date("F d, Y", strtotime($to)).'.xlsx"');
                header('Cache-Control: max-age=0');
                // If you're serving to IE 9, then the following may be needed
                header('Cache-Control: max-age=1');
