...
class Products_model extends CORE_Model {
    ...
    function get_inventory($as_of_date,$product_type_id=3,$is_show_all=TRUE,$supplier_id=0,$status=-1){
        $sql="SELECT
                ...
                m.*,
                p.product_desc,p.product_code,p.product_desc1,rp.product_type,s.supplier_name,c.category_name,
    ...
    function get_current_item_list_so($criteria="",$type=3){
        $sql = "SELECT 
                    ...
                    FORMAT(discounted_price, 4) AS srp_discounted,
                    FORMAT(purchase_cost_2, 2) AS cost_price,
                    IFNULL(FORMAT(rc.item_cost, 4), 0.00) AS srp_cost,
                    IFNULL((rc.in_qty - IFNULL(sinv.out_qty, 0) - IFNULL(iss.out_qty, 0) - IFNULL(aoQ.out_qty, 0)), 0) AS on_hand_per_batch
                FROM
                    products AS p
                LEFT JOIN
                    (SELECT
                    ...
                    GROUP BY inQ.product_id) AS rc ON p.product_id = rc.product_id
                LEFT JOIN
                    (SELECT
                        sii.product_id,
                        SUM(sii.inv_qty) AS out_qty
                    ...
                    INNER JOIN sales_invoice AS si ON sii.sales_invoice_id = si.sales_invoice_id
                    WHERE si.is_active = TRUE AND si.is_deleted = FALSE
                    GROUP BY sii.product_id) AS sinv ON p.product_id = sinv.product_id
                LEFT JOIN
                    (SELECT
                        iss.product_id,
                        SUM(iss.issue_qty) AS out_qty
                    ...
                    WHERE iin.is_active = TRUE AND iin.is_deleted = FALSE
                    GROUP BY iss.product_id) AS iss ON p.product_id = iss.product_id
                LEFT JOIN
                    (SELECT
                        aii.product_id,
                        SUM(aii.adjust_qty) AS out_qty
                    ...
                    INNER JOIN adjustment_info AS ai ON aii.adjustment_id = ai.adjustment_id
                    WHERE ai.adjustment_type = 'OUT' AND ai.is_active = TRUE AND ai.is_deleted = FALSE
                    GROUP BY aii.product_id) AS aoQ ON p.product_id = aoQ.product_id
                LEFT JOIN tax_types AS tt ON p.tax_type_id = tt.tax_type_id
                LEFT JOIN units AS u ON p.unit_id = u.unit_id
                WHERE p.is_active = TRUE AND p.is_deleted = FALSE AND ".($type==3?"":" p.refproduct_id=".$type." AND ")." (p.product_desc LIKE '%".$criteria."%' OR p.product_code LIKE '%".$criteria."%' OR p.product_desc1 LIKE '%".$criteria."%' OR CAST(p.product_id AS CHAR) LIKE '%".$criteria."%')"; // HAVING on_hand_per_batch>0
